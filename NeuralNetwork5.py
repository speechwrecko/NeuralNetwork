...
class MLP_NeuralNetwork(object):
    ...
    def __init__(self, input, hidden, output, iterations, learning_rate, momentum, rate_decay):
        ...
        # test init weights to 0.02
        #input_range = 1.0 / self.input ** (1 / 2)
        #output_range = 1.0 / self.hidden ** (1 / 2)
        #self.wi = np.random.normal(loc=0, scale=input_range, size=(self.input, self.hidden))
        #self.wo = np.random.normal(loc=0, scale=output_range, size=(self.hidden, self.output))
        self.wi = np.ones((self.input, self.hidden)) * 0.02
        self.wo = np.ones((self.hidden, self.output)) * 0.02
    ...
    def test(self, t_input, t_output):
        """
        Currently this will print out the targets next to the predictions.
        Not useful for actual ML, just for visual inspection.
        """
        for p, q in zip(t_input, t_output):
            #print(q, '->', self.feedForward(p))
            o = self.feedForward(p)
            print('ACTUAL:', np.array([q]).argmax(), '->', 'PREDICTED:', np.array([o]).argmax())
    ...
    def train(self, t_input, t_output):
        ...
        for i in range(self.iterations):
            ...
            #turn off random for test
            #randomize = np.arange(len(t_input))
            #np.random.shuffle(randomize)
            #t_input =t_input[randomize]
            #t_output = t_output[randomize]
...
def demo():
    ...
    T_IN, T_OUT = audio.LoadAudioTrainingDataFromFile('file_list.csv', 25, 13)
    #X = np.concatenate((C,D), axis=1)

    NN = MLP_NeuralNetwork(12, 100, 10, iterations=500, learning_rate=0.01, momentum=0.5, rate_decay=0.01)
